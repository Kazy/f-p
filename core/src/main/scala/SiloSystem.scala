...
trait SiloSystem extends SiloRefFactory with Logging {
  ...
  /** Returns the name of the silo system.
    *
    * If the silo system is running in server mode, [[name]] defaults to `Host:Port`.
    *
    * If the silo system is running is client mode, [[name]] defaults to a randomly generated [[java.util.UUID]].
    */
  def name: String
...
private[silt] trait Internals {
  ...
  // XXX With `Id` still required? If yes, mimic a set by `mutable.Map[Id, Unit]`
  //def location: mutable.Map[Id, Host] = new TrieMap[Id, Host]

  // XXX val refId = new AtomicInteger(0)
